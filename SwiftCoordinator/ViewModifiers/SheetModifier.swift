...
extension View {
    func sheetView<Item: Identifiable, Destination: View>(item: Binding<Item?>, destination: @escaping (Item) -> Destination) -> some View {
    modifier(SheetModifier(item: item, content: destination))
  }
}
